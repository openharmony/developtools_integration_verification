...
sys.path.append(os.path.join(BASE_DIR, 'acls_check'))
sys.path.append(os.path.join(BASE_DIR, 'APL_compare_03'))
...
def sys_exit():
    ...
    file_from_dev("/data/log/faultlog/temp/after_test_cppcrash{}.tar".format(args.device_num), os.path.normpath(args.save_path))
    enter_shell_cmd("cd /data/log/faultlog/faultlogger && tar -cf after_test_jscrash{}.tar jscrash*".format(args.device_num))
    file_from_dev("/data/log/faultlog/faultlogger/after_test_jscrash{}.tar".format(args.device_num), os.path.normpath(args.save_path))
...
def crop_picture(prefix, pic, crop_range):
    try:
        pic_path = os.path.join(args.save_path, '{}_{}'.format(prefix, pic))
        save_path = os.path.join(args.save_path, '{}_{}'.format(prefix, pic))
...
def cmp_picture(prefix, pic, num=1):
    if num == 1:
        img1_path = os.path.join(args.anwser_path, pic)
    else:
        img1_path = os.path.join(args.anwser_path, '2_{}'.format(pic))
    print('img1_path: {}'.format(img1_path))

    img2_path = os.path.join(args.save_path, '{}_{}'.format(prefix, pic))
    print('img2_path: {}'.format(img2_path))
    cmp_init = 0
    try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        lose_process = []
        process_pid = {}
        with open(os.path.join(args.tools_path, 'process.txt'), "r+") as f:
            text = f.read()
