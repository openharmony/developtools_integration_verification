...
def uninstall_hap(device):
    yield
    try:
        device.uninstall_hap('com.example.meituan')
    except:
        pass
...
class Test:
    ...
    bundle_name = 'com.example.meituan'

    @pytest.mark.parametrize('setup_teardown', [None], indirect=True)
    def test(self, setup_teardown, device):
        logging.info('install hap')
        hap_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(device.resource_path))), 'fangmeituan','entry-default-signed.hap')
